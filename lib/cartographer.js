...
Cartographer = (function() {
  ...
  Cartographer.prototype.apply = function(name, id, model, onMarkup, onError) {
    ...
    if (this.templates[name]) {
      ...
      return this.apply(templateName, id, model, onMarkup, onError);
    } else {
  ...
  Cartographer.update = function(id, controlId, model, onMarkup) {
    var template;
    if (this.containerLookup[id]) {
      template = this.containerLookup[id];
  ...
  Cartographer.watchEvent = function(id, event, onEvent) {
    var template;
    if (this.containerLookup[id]) {
      template = this.containerLookup[id];
  ...
  Cartographer.ignoreEvent = function(id, event) {
    var template;
    if (this.containerLookup[id]) {
      template = this.containerLookup[id];
...
Template = (function() {
  ...
  Template.prototype.apply = function(id, originalModel, onResult) {
    var model, self;
    self = this;
    ...
    if (!self.ready) {
      ...
      return self.render(id, model, id, void 0, function(x) {
        var result;
        result = {};
        if (!x.length) {
          result = $(x)[0];
