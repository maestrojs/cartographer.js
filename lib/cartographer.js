...
postal.subscribe("cartographer", "api.*", function(message, envelope) {
  if (envelope.topic === "api.map") {
    ...
    return cartographer.resolver.appendSource(message.provider);
  }
});
...
templateProperties = {
  ...
  var conditionalCopy, copyProperties, createFqn, externalTemplate, isCurrent, propertyCopy;
...
Template = function(id, name, model) {
  ...
  onElement = function(model, namespace, element, onDone, templates) {
    var child, childCallback, childrenCount, childrenToCreate, createElement, elementId, fqn, onChildElement, tag, _i, _len, _ref, _ref2, _ref3, _results;
    if (!element) {
