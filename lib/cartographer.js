...
Template = function(name) {
  ...
  buildCreateElement = function(element, elementId, elementFqn, childrenToCreate, templates) {
    ...
    return function(instance, model, modelFqn, id) {
      var callback, children, collection, controls, createElement, factory, indx, list, newFqn, newId, template, val, _ref;
      ...
      if (template) {
        ...
        if (self.templates[template]) {
          return self.templates[template](instance, model, modelFqn, id);
        } else {
          console.log("create element found template: " + template + " and has " + (JSON.stringify(templates)));
          callback = void 0;
          handleTemplate(template, template, function(x) {
            return crawl(elementFqn, x, templates, function(y) {
          ...
          return callback(instance, model, modelFqn, id);
        }
      } else {
        if (childrenToCreate && childrenToCreate.length > 0) {
          ...
          if (collection && collection.length) {
            ...
            self.factories[newFqn + "_add"] = function(newIndex, newModel) {
              ...
              for (_i = 0, _len = children.length; _i < _len; _i++) {
                factory = children[_i];
                _results.push(factory(instance, newModel, newFqn, newIndex));
              }
              return _results;
            };
            for (indx = 0, _ref = collection.length - 1; 0 <= _ref ? indx <= _ref : indx >= _ref; 0 <= _ref ? indx++ : indx--) {
              list.push((function() {
                ...
                for (_i = 0, _len = childrenToCreate.length; _i < _len; _i++) {
                  ...
                  _results.push(factory(instance, collection, newFqn, indx, modelFqn));
                }
                return _results;
              })());
            }
            return makeTag(tag, element, newFqn, newId, elementId !== void 0, list, model, instance);
          } else {
            controls = (function() {
              var _i, _len, _results;
              _results = [];
              for (_i = 0, _len = childrenToCreate.length; _i < _len; _i++) {
                ...
                _results.push(createElement(instance, val, newFqn, void 0, modelFqn));
              }
              return _results;
            })();
            return makeTag(tag, element, newFqn, newId, elementId !== void 0, controls, model, instance);
          }
        } else {
          return makeTag(tag, element, newFqn, newId, elementId !== void 0, val, model, instance);
        }
      }
    };
  };
  ...
  this.apply = function(id, originalModel, onResult) {
    ...
    if (!self.ready) {
      ...
      wireUp(id);
      return onResult(id, "render", result);
    }
  };
  ...
  this.update = function(fqn, model, onResult) {
    var childKey, lastIndex, newElement, parentKey, templateId;
    ...
    if (self.factories[fqn]) {
      newElement = self.factories[fqn](templateId, model, parentKey, childKey);
      return onResult(fqn, "update", newElement);
    }
  };
  this.add = function(fqn, model, onResult) {
    var addName, childKey, count, lastIndex, newElement, parentKey, templateId;
    lastIndex = fqn.lastIndexOf(".");
    templateId = fqn.split('.')[0];
    parentKey = fqn.substring(0, lastIndex);
    childKey = fqn.substring(lastIndex + 1);
