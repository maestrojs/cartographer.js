...
Template = function(name) {
  var buildCreateElement, crawl, createFqn, elementTemplate, handleTemplate, makeTag, self, wireUp;
  self = this;
  ...
  this.update = function(fqn, model, onResult) {
    var childKey, lastIndex, parentKey;
    lastIndex = fqn.lastIndexOf(".");
    parentKey = fqn.substring(0, lastIndex);
    childKey = fqn.substring(lastIndex + 1);
    if (self.factories[fqn]) {
      return self.factories[fqn](parentKey, model, parentKey, childKey, (function(dom) {
        var newElement;
        newElement = $(dom)[0];
        return onResult(fqn, newElement);
      }), "update");
    }
  };
  this.add = function(fqn, model, onResult) {
    var addName, count, listContainer;
    addName = fqn + "_add";
    if (self.factories[addName]) {
      listContainer = $("[" + configuration.elementIdentifier + "=\"" + fqn + "\"]");
      count = listContainer.length > 0 ? listContainer[0].children.length : 0;
