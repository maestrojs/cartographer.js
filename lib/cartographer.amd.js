...
Template = function(name) {
  ...
  crawl = function(namespace, markup, templates, onDone) {
    var addChildFunction, child, childFunctionsExpected, children, childrenFunctions, continueProcessing, elementFqn, elementId, elementIndex, index, list, onFactory, template, total, _i, _len, _ref, _ref2, _results, _results2;
    if ((markup != null ? markup.length : void 0) && (!markup.nodeType || markup.nodeType === 1)) {
      if (markup.length > 1) {
        total = markup.length;
        index = 0;
        list = new Array(total);
        onFactory = function(factory) {
          list[index++] = factory;
          console.log("got factory " + factory + " for index " + index);
          if (index === total) {
            return onDone(function(templateInstance, model, fqn, id, callback) {
              var fx, _i, _len, _results;
              _results = [];
              for (_i = 0, _len = list.length; _i < _len; _i++) {
                fx = list[_i];
                _results.push(fx(templateInstance, model, fqn, id, callback));
              }
              return _results;
            });
          }
        ...
        _results = [];
        for (elementIndex = 0, _ref = total - 1; 0 <= _ref ? elementIndex <= _ref : elementIndex >= _ref; 0 <= _ref ? elementIndex++ : elementIndex--) {
          _results.push(crawl(namespace, markup[elementIndex], templates, onFactory));
        }
        return _results;
      } else {
        ...
        if (template) {
          ...
          if (children.length > 0) {
            ...
            for (_i = 0, _len = children.length; _i < _len; _i++) {
              child = children[_i];
              _results2.push(crawl(elementFqn, child, templates, addChildFunction));
            }
            return _results2;
          } else {
  ...
  createChildren = function(iterations, templateInstance, model, modelFqn, id, context, onElement) {
    var childFqn, childId, childModel, children, create, index, isCollection, iteration, list, onChild, total, _ref, _results;
    ...
    onChild = function(childIndex, child) {
      list[index++] = child;
      if (index >= total) {
        return onElement(makeTag(context.tag, context.element, modelFqn, childIndex, context.elementId, list, model, templateInstance));
      }
    };
    _results = [];
    for (iteration = 0, _ref = iterations - 1; 0 <= _ref ? iteration <= _ref : iteration >= _ref; 0 <= _ref ? iteration++ : iteration--) {
      _results.push((function() {
        ...
        for (_i = 0, _len = children.length; _i < _len; _i++) {
          ...
          _results2.push(create(templateInstance, childModel, childFqn, childId, function(x) {
  ...
  create = function(templateInstance, model, modelFqn, id, context, onElement) {
    var childElementCount, childElements, childModel, collection, collectionLength, elementId, idForFqn, isBound, isCollection, memberTemplate, memberValue, modelTemplate, newFqn, template, _ref;
    ...
    if (template) {
      return handleModelTemplate(template, templateInstance, model, modelFqn, id, context, onElement);
    } else {
